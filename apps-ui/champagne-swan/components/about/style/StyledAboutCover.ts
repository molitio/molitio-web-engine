...
export const StyledAboutMainTitle = styled.h1`
    ...
    text-shadow: ${(props) => props?.theme?.palette?.visual['aboutCoverTitleShadow']?.textShadow ?? ''};
...
export const StyledAboutSubTitle = styled.h2`
    ...
    margin: 0;
    color: ${(props) => props?.theme?.palette?.text?.secondary ?? ''};
    text-shadow: ${(props) => props?.theme?.palette?.visual['aboutCoverSubTitleShadow']?.textShadow ?? ''};
...
export const StyledBoutCoverImage = styled.img`
    object-fit: cover;
`;
