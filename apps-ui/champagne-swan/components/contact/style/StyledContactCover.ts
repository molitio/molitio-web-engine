...
export const StyledAboutContentCoverImage = styled.img`
    object-fit: cover;
`;
...
export const StyledContactCoverTitle = styled.h1`
    ...
    text-shadow: ${(props) => props?.theme?.palette?.visual['contactCoverTitleShadow']?.textShadow ?? ''};
    font-weight: 400;
    font-size: 3.8em;
    margin-top: 0.4em;
...
export const StyledCOntactCoverDescription = styled.text`
    ...
    color: ${(props) => props?.theme?.palette?.text?.secondary ?? ''};
    text-shadow: ${(props) => props?.theme?.palette?.visual['contactCoverTextShadow']?.textShadow ?? ''};
